<#
.Synopsis
  Provides access to Windows CredentialManager basic functionality for client scripts
...
* 
* 	.\CredentialManager.ps1 -AddCred -Target 'http://server' -User 'JoeSchmuckatelli' -Pass 'P@55w0rd!'
* 
...
.Description
  ...
  DeleteCredential (requires -Target)
  GetCred (requires -Target)
  RunTests (no cmd-line opts)
  ShowCredential (optional -All parameter to dump cred objects to console)
...
.OUTPUTS
  ...
  ** Successful Action **
  * Del-Cred   : Int = 0
  * Enum-Cred  : PsUtils.CredentialManager+Credential[]
  * Read-Cred  : PsUtils.CredentialManager+Credential
  * Write-Cred : Int = 0
  ** Failure **
...
.PARAMETER RunTests
	Specifies that you wish to run built-in Win32 CredentialManager functionality tests

.PARAMETER ShowCredential
...
.EXAMPLE
	.\CredentialManager.ps1 -AddCred -Target 'http://aserver' -User 'UserName' -Password 'P@55w0rd!' -Comment 'cuziwanna'
...
(
	...
	[Parameter(Mandatory=$false)][Switch] $DeleteCredential,
	[Parameter(Mandatory=$false)][Switch] $GetCred,
	[Parameter(Mandatory=$false)][Switch] $ShowCredential,
...
namespace PsUtils
{
    public class CredentialManager
    {
        ...
        public static void Main()
        {
            ...
            Rtn = CredEnum(null, out Credentials);
...
$PsCredentialManager = $null
try
{
	$PsCredentialManager = [PsUtils.CredentialManager]
}
catch
{
...
if($null -eq $PsCredentialManager)
{
...
#>
	...
	[PsUtils.CredentialManager+Credential[]] $Credentials = [Array]::CreateInstance([PsUtils.CredentialManager+Credential], 0)
	[Int] $Results = 0
	try
	{
		$Results = [PsUtils.CredentialManager]::CredEnum($Filter, [Ref]$Credentials)
	}
	catch
	{
		return $_
...
.Description
  Calls Win32 CredWriteW via [PsUtils.CredentialManager]::CredWrite
...
#>
	...
	{
		$true  {$Cred.Flags = [PsUtils.CredentialManager+CRED_FLAGS]::USERNAME_TARGET}
		$false  {$Cred.Flags = [PsUtils.CredentialManager+CRED_FLAGS]::NONE}
	}
...
function CredentialManagerMain
{
#region Adding credentials
	if($AddCred)
	{
		...
		[Object] $Results = Write-Credentials $Target $User $Pass $Comment $CredType $CredPersist
		if(0 -eq $Results)
		{
			[Object] $Cred = Read-Credentials $Target $CredType
			if($null -eq $Cred)
			{
...
#region Removing credentials
	if($DeleteCredential)
	{
		...
		# may be [Int32] or [Management.Automation.ErrorRecord]
		[Object] $Results = Del-Credentials $Target $CredType 
		if(0 -eq $Results)
		{
...
#region Reading selected credential
	if($GetCred)
	{
		...
		# may be [PsUtils.CredentialManager+Credential] or [Management.Automation.ErrorRecord]
		[Object] $Cred = Read-Credentials $Target $CredType
		if($null -eq $Cred)
		{
...
#region Reading all credentials
	...
	{
		# may be [PsUtils.CredentialManager+Credential[]] or [Management.Automation.ErrorRecord]
		[Object] $Credentials = Enum-Credentials
		if($Credentials -split [Array] -and 0 -eq $Credentials.Length)
		{
			Write-Host "No Credentials found for $($Env:UserName)"
			return
		}
		if($Credentials -is [Management.Automation.ErrorRecord])
		{
			return $Credentials
		}
...
#region Run basic diagnostics
	if($RunTests)
	{
		[PsUtils.CredentialManager]::Main()
	}
#endregion
}
#endregion

CredentialManagerMain
